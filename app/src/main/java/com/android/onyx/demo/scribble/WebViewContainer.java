...
public class WebViewContainer extends WebView {
    ...
    public void setTouchHelper(final TouchHelper touchHelper) {
        this.touchHelper = touchHelper;
    }

    public void setPenButton(final ImageButton buttonPen) {
        this.penButton = buttonPen;
    }
    ...
    public WebViewContainer(final Context context) {
        super(context);
...
//                    emptyAreas.clear();
//                    emptyAreas.add(new EmptyArea(mEmptyAreaStart.y + super.getScrollY(), mEmptyAreaEnd.y - mEmptyAreaStart.y));
//                    invalidate();
//                    return null;
//                };
...
Н
        if (canvas == webViewCanvas) {
            super.onDraw(canvas);
            return;
        }
        webViewCanvas.translate(0, -super.getScrollY());
        super.draw(webViewCanvas);
        webViewCanvas.translate(0, super.getScrollY());
//        super.onDraw(canvas);
...
//            int emptyAreaHeight;
//            if (super.getScrollY() < emptyArea.y) {
//                webViewCursor = emptyArea.y - super.getScrollY();
...
//            } else {
//                webViewCursor = 0;
//                emptyAreaHeight = Math.max(emptyArea.height - (super.getScrollY() - emptyArea.y), 0);
//            }
//
...
//        Rect remainingDest = new Rect(0, webViewCursor + emptyAreasTotalHeight, getWidth(), getHeight());
        canvas.drawBitmap(webviewBitmap, 0, 0, strokesPaint);

//        for (final Path path : paths) {
//            Path pathToDraw = new Path(path);
//            pathToDraw.offset(0, -super.getScrollY());
...
//        }
    ...
    private Path createPath(final List<TouchPoint> points) {
        ...
        path.offset(0, super.getScrollY());
        return path;
    }
}
